...
pub fn calendar_add_event_processor(router_input: RouterInput, processor_data: Arc<processing::ProcessorData>) -> RouterOutput {
    ...
    match request_model_de {
      Ok(request_model) => {
        ...
        match calendar::calendar_add_event(request_model, session_token_model, processor_data) {
            None => {
                RouterOutput{response_body: serde_json::to_string(&calendar_model::CalendarAddEventResponseModel {
                    error: None,
                }).unwrap()}
            }
            Some(e) => {
