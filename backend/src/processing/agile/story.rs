...
pub fn lookup_backlog_processor(
    ...
    match decode_router_input_to_model!(story_model::LookupBacklogRequestModel, router_input) {
        Ok(request_model) => {
            validate_session!(processor_data, request_model);

            match story::lookup_backlog(request_model, processor_data) {
                Ok(response) => {
                    model_to_router_output!(response)
                },
                Err(e) => {
                    model_to_router_output!(story_model::LookupBacklogResponseModel {
                        stories: Vec::new(),
                        error: service_error_to_model!(EvelynServiceError::LookupBacklogAgileStories(e)),
                    })
                },
            }
        },
